...
public class WorldGenGysahlGreen implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
		if (!world.provider.isSurfaceWorld() || !world.provider.hasSkyLight()) return;
		...
		if (random.nextFloat() > ChocoConfig.world.gysahlGreenSpawnChance) return;
		...
		for (int i = 0; i < ChocoConfig.world.gysahlGreenPatchSize; ++i) {
			BlockPos blockPos = finalPosition.add(random.nextInt(8) - random.nextInt(8), random.nextInt(4) - random.nextInt(4), random.nextInt(8) - random.nextInt(8));
