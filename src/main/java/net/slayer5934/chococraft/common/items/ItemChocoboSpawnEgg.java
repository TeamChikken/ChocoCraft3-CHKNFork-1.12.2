...
public class ItemChocoboSpawnEgg extends Item implements IItemModelProvider {
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (entity instanceof EntityChocobo) {
			...
			if (player.isSneaking()) entityliving.setGrowingAge(-24000);
	...
	public String getTranslationKey(ItemStack stack) {
		...
		if (meta >= 0 && meta < ChocoboColor.values().length) return super.getTranslationKey(stack) + "." + ChocoboColor.values()[meta].name().toLowerCase();
