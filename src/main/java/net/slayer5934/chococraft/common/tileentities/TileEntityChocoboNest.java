...
public class TileEntityChocoboNest extends TileEntity implements ITickable {
	...
	public void update() {
		...
		if (changed) this.world.setBlockState(this.pos, this.world.getBlockState(this.pos));
	}

	private boolean updateEgg() {
		ItemStack egg = this.getEggItemStack();
		if (!egg.hasTagCompound()) return false;
		...
		if (time < ChocoConfig.breeding.eggHatchTimeTicks) return false;
	...
	private boolean updateSheltered() {
		...
		for (CheckOffset checkOffset : SHELTER_CHECK_OFFSETS) {
			if (world.isAirBlock(this.getPos().add(checkOffset.offset)) != checkOffset.shouldBeAir) {
				if (this.isSheltered) changed = true;
	...
	public void setEggItemStack(ItemStack itemStack) {
		...
		else if (BlockChocoboEgg.isChocoboEgg(itemStack)) this.inventory.setStackInSlot(0, itemStack);
	}
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		if (nbt.hasKey(NBTKEY_OWNER_CHOCOBO)) this.ownerChocobo = NBTUtil.getUUIDFromTag(nbt.getCompoundTag(NBTKEY_OWNER_CHOCOBO));
	...
	public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
		...
		if (this.ownerChocobo != null) nbt.setTag(NBTKEY_OWNER_CHOCOBO, NBTUtil.createUUIDTag(this.ownerChocobo));
