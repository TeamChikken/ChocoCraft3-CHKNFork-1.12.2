...
public class EntityChocobo extends EntityTameable {
	...
	public void readEntityFromNBT(NBTTagCompound nbt) {
		...
		if (getSaddleType() != SaddleType.NONE) this.chocoboInventory.deserializeNBT(nbt.getCompoundTag(NBTKEY_INVENTORY));
		
		if (nbt.hasKey(NBTKEY_NEST_POSITION)) this.nestPos = NBTUtil.getPosFromTag(nbt.getCompoundTag(NBTKEY_NEST_POSITION));
	...
	public void writeEntityToNBT(NBTTagCompound nbt) {
		...
		if (getSaddleType() != SaddleType.NONE) nbt.setTag(NBTKEY_INVENTORY, this.chocoboInventory.serializeNBT());
		
		if (this.nestPos != null) nbt.setTag(NBTKEY_NEST_POSITION, NBTUtil.createPosTag(this.nestPos));
			...
			if (this.canPassengerSteer()) {
				if (rider.isJumping) {
					if (rider.isSprinting() && this.canFly() && !rider.isInWater() && this.useStamina(ChocoConfig.chocobo.flyStaminaCost)) {
						...
						if (motionY > 0.5f) this.motionY = 0.5f;
				...
				if (rider.isInWater()) {
					if (this.canDive()) {
						if (rider.isSneaking()) {
							this.motionY -= 0.05f;
							if (this.motionY < -0.7f) this.motionY = -0.7f;
						}
						...
						if (rider.isJumping) {
							...
							if (distance > 0) this.motionY = .01f + Math.min(0.05f * distance, 0.7);
						}
					}
				}
	...
	private void regenerateStamina() {
		...
		if (!this.onGround && !this.isInWater() && !this.isInLava() && !this.isSprinting()) return;
		
		float regen = ChocoConfig.chocobo.staminaRegenRate;
		
		// half the amount of regeneration while moving
		if (this.motionX != 0 || this.motionZ != 0) regen *= 0.85;
	...
	public boolean processInteract(EntityPlayer player, EnumHand hand) {
		...
		if (this.isTamed() && player.isSneaking() && !this.isChild()) {
			if (player instanceof EntityPlayerMP) this.displayChocoboInventory((EntityPlayerMP) player);
			return true;
		}
	...
	private void reconfigureInventory(SaddleType oldType, SaddleType newType) {
		...
		for (EntityPlayer player : world.playerEntities) {
			if (player.openContainer instanceof ContainerSaddleBag) ((ContainerSaddleBag) player.openContainer).refreshSlots(this, player);
		}
	}
	...
	protected void dropLoot(boolean wasRecentlyHit, int lootingModifier, DamageSource source) {
		...
		if (this.chocoboInventory != null && this.isSaddled()) {
			for (int i = this.chocoboInventory.getSlots() - 1; i >= 0; i--) {
				if (!this.chocoboInventory.getStackInSlot(i).isEmpty()) this.entityDropItem(this.chocoboInventory.getStackInSlot(i), 0.0f);
			}
		}
	}
